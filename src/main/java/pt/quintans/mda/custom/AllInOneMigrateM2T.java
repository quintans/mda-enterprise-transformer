...
public class AllInOneMigrateM2T extends Model2TextAbstract {
	...
	public void transform(List<Object> mappings) {
		...
		if(allModels.length > 1){
		    ...
		    String destinationFolder = getDestination();
		    String destinationFile = getFilename();
			
		    putInPipe(PipelineKeys.DESTINATION_FOLDER, destinationFolder);
		    
		    if(!work.isQuiet())
		    	System.out.println(String.format("A gerar <<%s>> [%s] => %s/%s", getStereotype(), getTemplate(), destinationFolder, destinationFile));						

		    dumpToFile(destinationFolder, destinationFile);
		}  
